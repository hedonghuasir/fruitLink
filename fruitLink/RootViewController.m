...
-(void)createStartAndStop
{
    ...
    buttonStart.frame = CGRectMake(50, 400, 60, 30);
    ...
    buttonStop.frame = CGRectMake(210, 400, 60, 30);
...
-(void)createButtons
{
    ...
    for(int i=0;i<Row;i++)//行
    {
        for(int j=0;j<Col;j++)//列
        {
            ...
            //设置标志
            button.tag = i*10+j+1;//从1开始
            //设置大小
            button.frame = CGRectMake(j*30+10,i*30+50, 30, 30);
...
-(void)buttonClick:(UIButton*)curButton
{
    ...
    else//当前按钮不为空
    {
        ...
        if(_LastButton.currentBackgroundImage == _curButton.currentBackgroundImage)//背景图片是否相同
        {
            ...
            else  //不是同一个按钮
            {
                ...
                if([self linkSuccess])//            线路走的同成功
                {
                    ...
                    int last_x = (int)_LastButton.tag/10;
                    if(_LastButton.tag / 10 == 10)//最后一行
                    {
                        last_x = Row - 1;
                    }
                    int last_y = _LastButton.tag%10-1;
                    if(_LastButton.tag %10 == 0)//最后一列
                    {
                        last_y = Col - 1;
                        last_x = (int)_LastButton.tag/10 - 1;
                    }
                    //current
                    int cur_x = (int)_curButton.tag/10;
                    if(_curButton.tag /10 == 10)
                    {
                        cur_x = Row - 1;//最后一行
                    }
                    int cur_y = _curButton.tag%10 - 1;
                    if(_curButton.tag %10 == 0)
                    {
                        cur_y = Col - 1;
                        cur_x = (int)_curButton.tag/10 - 1;
                    }
                ...
                else//走不通 怎么处理
                {
                    _curButton  = curButton;
                    _LastButton.backgroundColor = [UIColor clearColor];
                    _curButton.backgroundColor = [UIColor blackColor];
                    _LastButton = nil;
                }
...
-(void)startGame:(UIButton*)startButton
{
    ...
    for(int i=0;i<Row;i++)
        ...
        {
            map[i][j] = 1;
            UIButton* button = (UIButton*)[self.view viewWithTag:((i*10)+j+1)];
...
-(BOOL)linkSuccess
{
    ...
    //获取连个  按钮的坐标
    //***************************************************************左上方按钮坐标处理
    int min_x = (int)minButton.tag/10;
    if(minButton.tag / 10 == 10)//  最后一行
    {
        min_x = Row - 1;
    }
    //最后一列的坐标  特殊  处理
    int min_y = minButton.tag%10-1;
    if(minButton.tag % 10 == 0)
    {
        min_y = Col - 1;
        min_x = (int)minButton.tag/10 - 1;//为最后一列，  X坐标要减去  1
    }
    //***************************************************************右下方按钮坐标处理
    //最后一列的坐标  特殊  处理
    int max_x = (int)maxButton.tag/10;
    if(maxButton.tag / 10 == 10)//  最后一行
    {
        max_x = Row - 1;
    }
    //y坐标
    int max_y = maxButton.tag%10-1;
    if(maxButton.tag % 10 == 0)
    {
        max_y = Col - 1;
        max_x = (int)maxButton.tag/10 - 1;//如果是最后一列  那么要 x坐标要  减去 1
    }
